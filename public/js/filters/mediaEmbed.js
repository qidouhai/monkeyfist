...
angular.module('internal').filter('mediaEmbed', ['$sce', function ($sce) {
    ...
    function insertEmoji(str) {
        ...
        str = str.replace(new RegExp("(" + Object.keys(emojiCodes).map(function (i) {
                return i.replace(/[()*<>$]/g, "\\$&");
            }).join("|") + ")", "g"), function (s) {
            return emojiCodes[s];
        });
    ...
    function youtubeEmbed(str) {
        ...
        if (str.match(ytRegex)) {
            var iframe = '<div class="row"><div class="col-md-offset-2 col-md-8 col-xs-12"><div class="embed-responsive embed-responsive-16by9"><iframe width="400" class="embed-responsive-item" src="' + $sce.trustAsResourceUrl('https://www.youtube.com/embed/' + RegExp.$1 + '?autoplay=0') + '" allowfullscreen="allowfullscreen"></iframe></div></div></div>';
